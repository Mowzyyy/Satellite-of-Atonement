function scrMove(_dir){
    ...
    if (state == states.idle) {
		...
		var _barrier = scrIsBarrierBlocked(dest_x, dest_y, _dir);
		var _npc = scrGetBlockingNpc(dest_x, dest_y);
		
		if (_npc != noone) {
			//npc blocks - hard bump is a stop, not a slide
			last_dir = _dir;
			sprite_index = sprite_standing;
			image_index = _dir;
			global.bumped_npc = _npc;//remember who was bumped for interaction
			return;
		}
		
		if (_barrier) {
			show_debug_message("BLOCKED at " + string(dest_x) + "," + string(dest_y) + " from dir " + string(_dir));
		 //try to slide (barriers only)
		 ...
		 if (slide_dir != -1) {
			 ...
			 //hard block - face direction but don't move
			 last_dir = _dir;
			 sprite_index = sprite_standing;
			 image_index = _dir;
			 }
